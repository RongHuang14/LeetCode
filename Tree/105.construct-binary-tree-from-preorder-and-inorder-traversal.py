...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        # dfs构建子树
        # 参数含义:
        # preorder[pre_l:pre_r] 这一段是当前子树的前序遍历
        # inorder[in_l:in_r]    这一段是当前子树的中序遍历
        def dfs(pre_l, pre_r, in_l, in_r):
            if pre_l == pre_r:
                return None
            root_val = preorder[pre_l] # 根还是这一段前序的第一个
            i = index[root_val] # 根在整条inorder中的位置
            left_size = i - in_l        # 左子树长度=根在当前中序区间中的偏移量
            
            # 左子树:
            # 前序: 紧跟在根后面的left_size个
            # 中序: in_l到i这一段
            left = dfs(pre_l+1, pre_l+1+left_size,in_l,i)
            
            # 右子树:
            # 前序: 左子树之后的那段
            # 中序: 根i之后到in_r这一段
            right = dfs(pre_l+1+left_size,pre_r,i + 1, in_r)
            
            return TreeNode(root_val, left, right)
        n = len(preorder)
        return dfs(0, n, 0, n) # 一开始前序中序都是整段
